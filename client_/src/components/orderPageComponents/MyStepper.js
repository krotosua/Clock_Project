...
const MyStepper = observer(({alertMessage}) => {
    ...
    const handleNext = () => {
        if (activeStep === 0) {
            setLoading(true)
            fetchMastersForOrder(cities.selectedCity, date, time, size.selectedSize.id, masters.page, 3).then((res) => {
                if (res.status === 204) {
                    setFreeMasters([])
                    return
                }
            ...
            createOrder(body).then(res => {
                ...
                setActiveStep(1)
                setLoading(true)
                fetchMastersForOrder(cities.selectedCity, date, time, size.selectedSize.id).then((res) => {
                    if (res.status === 204) {
                        setFreeMasters([])
                        return
                    }
    ...
    const checkInfo = !name || !email || !date || !time || !cities.selectedCity || !size.selectedSize.id || reg.test(email) === false || name.length < 3 || errorTimePicker || errorDatePicket


    useEffect(() => {
        if (activeStep === 1) {
            setLoading(true)
            fetchMastersForOrder(cities.selectedCity, date, time.toLocaleTimeString(), size.selectedSize.id, masters.page, 3).then((res) => {
                if (res.status === 204) {
                    setFreeMasters([])
                    return
                }
    ...
    return (<Box sx={{width: "100%"}}>
        ...
        {activeStep === 0 ? (
            ...
            activeStep === steps.length - 2 ? (<Box sx={{mt: 2, position: "relative"}}>
                {loading ? (<Box sx={{
                    display: "flex", justifyContent: "center", alignItems: "center",
                }}>
                    <CircularProgress/>
                </Box>) : freeMasters.length == 0 ? (<Typography variant="h4" sx={{my: 2, textAlign: "center"}}>
                    Все мастера заняты
                </Typography>) : (<Box sx={{flexGrow: 1, maxWidth: "1fr"}}>
                    <Typography sx={{my: 4,}}>
                        Свободные мастера
                    </Typography>
                    <List disablePadding>
                        ...
                        >
                            {freeMasters.length == 0 ? (<Typography sx={{mb: 2}}>
                                Все мастера заняты
                            </Typography>) : (freeMasters.map((master, index) => {
                                let isFree = true
                                return (<ListItem key={master.id}
                                                  divider
                                                  style={{cursor: 'pointer'}}
                                                  selected={chosenMaster == master.id}
                                                  onClick={() => choseMaster(null, master.id)}
                                                  secondaryAction={isFree ? <Tooltip title={'Выбрать мастера'}
                                                                                     placement="right"
                                                                                     arrow>
                                                      <FormControlLabel
                                                          value={master.id}
                    ...
                    {loading ? <CircularProgress size={30}
                                                 sx={{
                                                     position: "absolute", right: 5, bottom: 85
                                                 }}/> : ""}
                </Box>)}
